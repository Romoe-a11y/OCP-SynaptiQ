...
function computeKpis(data: DashboardResponse | null) {
  ...
  // Fleet health based on latest status and alert ratio
...
export default function AdminReportPage() {
  ...
  const executiveSummary = useMemo(() => {
    ...
    if (riskLevel === "CRITICAL") {
      return `Immediate attention required. ${machine} is operating in a critical state with ${openAlerts} open alert${openAlerts !== 1 ? "s" : ""}. Fleet health stands at ${fleetHealth}%. Recommend immediate intervention and root cause analysis.`;
    }
    if (riskLevel === "HIGH") {
      return `Elevated risk detected. ${machine} shows warning indicators with ${openAlerts} active alert${openAlerts !== 1 ? "s" : ""}. Fleet health is at ${fleetHealth}%. Preventive maintenance should be prioritized.`;
    }
    if (riskLevel === "MODERATE") {
      return `Moderate operational conditions. Minor anomalies detected across the fleet. Fleet health is at ${fleetHealth}%. Continue standard monitoring protocols with increased observation frequency.`;
    }
    return `All systems operating within normal parameters. Fleet health is at ${fleetHealth}% with no active alerts. Standard monitoring and maintenance schedules are adequate.`;
  }, [kpis]);
  ...
  if (loading) {
    return (
      <div className="report-loading-page">
        <div className="report-loading-box">Preparing executive report...</div>
      </div>
    );
  }
      ...
      <main className="report-page exec-report">
        ...
        <section className="exec-meta">
          ...
          <div className="exec-meta-item">
            ...
            <span className="exec-meta-value">Administration</span>
          </div>
          <div className="exec-meta-item">
        ...
        <section className="exec-section">
          ...
          <p className="exec-body-text">{executiveSummary}</p>
        </section>
        ...
        <footer className="exec-footer">
          ...
          <div className="exec-footer-content">
            <span>
              <strong>OCP SynaptiQ</strong> — Confidential
            </span>
            <span>
              Generated {formatDate(new Date().toISOString())} — Administrator Space
            </span>
          </div>
        </footer>
      </main>
